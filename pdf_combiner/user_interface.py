...
class PdfCombiner(QMainWindow, QtStyleTools):
    def __init__(self):
        ...
        self.crope: str = DEFAULT_SETTINGS['crope']
        self.spacing: str = DEFAULT_SETTINGS['spacing']
    ...
    def _save_settings(self) -> None:
        quantity = self.ui.settings_window.choose_quantity.value()
        page_size = self.ui.settings_window.choose_page_size.currentText()
        crope = self.ui.settings_window.crope_form.text()
        spacing = self.ui.settings_window.spacing.text()

        if not crope.strip():
            pass
        elif 0 < len(crope.split(',')) < 4:
            self.message_box.warning(
                self, 'Предупреждение',
                'Поле с координатами должно содержать четыре координаты!')
            return
        elif any(not val.isdecimal() for val in crope.split(',')):
            self.message_box.warning(
                self, 'Предупреждение',
                'Поле с координатами должно содержать число!')
            return
    ...
    def _winrar_combine(self, rar_path_file: str) -> None:
        ...
        try:
            ...
            create_combine_pdf(
                ...
                crope=self.crope, spacing=float(self.spacing))

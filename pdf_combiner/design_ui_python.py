...
from PySide6.QtWidgets import (
    ...
    QWidget, QVBoxLayout, QLabel, QComboBox, QSpinBox, QProgressBar, QLineEdit
)
...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.settings_pdf = QLabel(
            ...
            f'Количестов элементов на странице: <u>{MainWindow.quantity}</u>',
            self.centralwidget)
...
class SettingsWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        label_quantity.setGeometry(QRect(13, 76, 240, 35))
        ...
        label_crope = QLabel(
            'Координаты элемента\n'
            '(только если не работает автоматически!)\n'
            'левый вверх = 0,134 (x, y)\n'
            'правый низ = 123,567 (x, y)',
            self)
        label_crope.setGeometry(QRect(13, 160, 360, 60))
        label_crope.setStyleSheet('color: white; \n'
                                  'font-weight: bold; \n'
                                  'font-size: 10pt;')

        self.crope_form = QLineEdit(self)
        self.crope_form.setGeometry(QRect(12, 230, 120, 35))
        self.crope_form.setPlaceholderText('0,134,123,567')
        self.crope_form.setText(parent.crope)

        label_spacing = QLabel('Дистанция между элементами', self)
        label_spacing.setGeometry(QRect(13, 259, 360, 35))
        label_spacing.setStyleSheet('color: white; \n'
                                    'font-weight: bold; \n'
                                    'font-size: 10pt;')

        self.spacing = QLineEdit(self)
        self.spacing.setGeometry(QRect(12, 295, 120, 35))
        self.spacing.setPlaceholderText('0.01')
        self.spacing.setText(parent.spacing)
